...
parser.add_argument('--checkpoint-dir', default='runs/pretrain/2/', type=Path,metavar='DIR', help='path to checkpoint directory')
...
class BYOL(nn.Module):
    ...
    def graph(self,data):
        ...
        data2=torch.sqrt(data1.sum(1)).mean(3)
        data3=data2.mean(1)
        data3=F.normalize(data3, dim=1)
        return data3
    #Train
    def forward(
        self,
       ...
       # p2 = h(z2)
        ...
        with torch.no_grad():
            ...
            x11 = nn.functional.normalize(x11,dim=1)
            x22 = target_encoder(x2,return_projection = True)
            x22 = nn.functional.normalize(x22,dim=1)
            x11.detach_()
            x22.detach_()
            
    #    loss_one = loss_fn(p1, x22.detach())
      #  loss_two = loss_fn(p2, x11.detach())
       
        B,C=z1.size()
        score=torch.abs(x22.unsqueeze(1)-self.queue.T.unsqueeze(0)).mean(2)
        _,index=torch.topk(score,25600)
        index=index.unsqueeze(1).expand(B,C,25600)
       ...
       # loss_1 = - torch.sum(F.softmax(logitsk.detach() / self.tt, dim=1) * F.log_softmax(logitsq / self.ot, dim=1), dim=1).mean()
        l_pos = torch.einsum('nc,nc->n', [z1, x22.detach()]).unsqueeze(-1)
        l_neg = torch.einsum('nc,nck->nk', [z1, self.queue2.clone().detach()])
        logits = torch.cat([l_pos, l_neg], dim=1)
        logits /= self.tt
        labels = torch.zeros(logits.shape[0], dtype=torch.long).cuda()
        loss_1=self.criterion(logits,labels)
        #print(loss_1)
        self._dequeue_and_enqueue(x22)
