...
def openpose_match(data_numpy):
    ...
    return data_numpy
def courruption(input_data):                                                                     

    out = input_data.copy()

    flip_prob  = random.random()
    right=[8, 9, 10, 11, 23, 24, 16, 17, 18, 19,1]
    left=[4, 5, 6, 7, 21, 22, 12, 13, 14, 15,1]
    if flip_prob < 0.5:

        #joint_indicies = np.random.choice(25, random.randint(5, 10), replace=False)
        joint_indicies = np.random.choice(right, 10,replace=False)
     #   print(joint_indicies)
        out[:,:,joint_indicies,:] = 0 
    else:
         joint_indicies = np.random.choice(left, 10,replace=False)
         out[:,:,joint_indicies,:] = 0 
    return out
