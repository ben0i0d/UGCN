...
parser.add_argument('--learning-rate', default=2, type=float, metavar='LR',
                    help='base learning rate')
parser.add_argument('--weight-decay', default=1e-6, type=float, metavar='W',
                    help='weight decay')
parser.add_argument('--print-freq', default=200, type=int, metavar='N',
                    help='print frequency')
parser.add_argument('--checkpoint-dir', default='./xsub60/1/', type=Path,
...
def main_worker(gpu, args):
   ...
   # ckpt = torch.load(args.checkpoint_dir / 'best.pth', map_location='cpu')
    start_epoch = 0
   # model.load_state_dict(ckpt['model'], strict=False)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch+1) % 10 == 0:
            torch.save(model.encoder.state_dict(),
                       args.checkpoint_dir / 'stgcn_{}.pth'.format(epoch + 1))

    torch.save(model.encoder.state_dict(),
                args.checkpoint_dir / 'stgcn.pth')
